...
inline off_t ReadAction::get_offset() {
...
ReadAction::ReadAction(std::chrono::milliseconds monitoring_interval,
...
#  warning "Unknown system"
  ...
  if (is_time_based()) {
    ...
    spdlog::debug("{}::work: interval: {}", typeid(*this).name(),
...
void ReadAction::log_values() { addMonitoredData("read_bytes", m_read_bytes); }

inline off_t RandomReadAction::get_offset() {
  static off_t block_size_bytes = get_block_size().convert<DataUnit::B>().get_value();
  static ssize_t file_size_bytes = get_file_size().convert<DataUnit::B>().get_value();
...
inline off_t WriteAction::get_offset() {
  static off_t offset = 0;
  static off_t block_size_bytes = get_block_size().convert<DataUnit::B>().get_value();
  static ssize_t file_size_bytes = get_file_size().convert<DataUnit::B>().get_value();
...
inline void WriteAction::generate_random_chunk(char* chunk, size_t size) {
...
WriteAction::WriteAction(std::chrono::milliseconds monitoring_interval,
                         std::function<void(VirtualMonitoredAction*)> on_log,
                         FileActionAttributes file_attributes)
    : VirtualMonitoredAction(monitoring_interval, on_log), FileActionAttributes(file_attributes) {}

void WriteAction::work() {
  spdlog::debug("{}::work: file_path: {}", typeid(*this).name(), get_file_path());
  int fd;
  std::unique_ptr<char[]> line(new char[get_block_size().convert<DataUnit::B>().get_value()]);
...
#  warning "Unknown system"
  ...
  if (fd == -1) {
    ...
    if (errno == EINVAL) {
      throw std::runtime_error("WriteAction::work: error Direct IO not supported");
    }
...
void WriteAction::log_values() { addMonitoredData("write_bytes", m_written_bytes); }

inline off_t RandomWriteAction::get_offset() {
  static off_t block_size_bytes = get_block_size().convert<DataUnit::B>().get_value();
  static ssize_t file_size_bytes = get_file_size().convert<DataUnit::B>().get_value();
